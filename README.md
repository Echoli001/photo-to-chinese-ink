...
Photo to Chinese Ink 把用户上传的照片，或写的一段文字场景描述，转成一段吴冠中风格的现代水墨画 prompt，可以直接粘贴进 Seedance, Image, Midjourney、即梦等文生图工具使用。整个流程分三步、且是确定性逻辑：诊断构图与主体 → 按 13 类主体路由表匹配风格规则 → 编译成最终 prompt，内置多重自查（防止水彩化描述、禁用措辞、签名印章等）。Skill 自身的诊断/路由/编译逻辑只产出文本，不调用任何图像生成 API；但如果触发这次对话的宿主会话本身已经挂了可调用的原生文生图工具，模型可以在完成三步后自己调用那个工具直接出图，只展示最终水墨图片，跳过中间的诊断卡/路由/prompt 展示——具体条件和回退方式见 [`SKILL.md`](.claude/skills/photo-to-chinese-ink/SKILL.md) 第 4 步。

Photo to Chinese Ink turns an uploaded photo — or a written scene description — into a Wu Guanzhong-style modern ink-wash art prompt, ready to paste into an image generator like Seedance, Image, Midjourney or 即梦. It works in three deterministic steps: diagnose the image's composition and subject, route it through a 13-category style table, and compile a final prompt with built-in guardrails against watercolor-style output, forbidden phrasing, and signature/seal artifacts. The skill's own logic (diagnose/route/compile) never calls any image-generation API — it only produces text. If the host session already has a native image-generation tool available, the assistant may call that tool itself with the compiled prompt and show only the resulting image, skipping the intermediate diagnosis/routing/prompt display — see [`SKILL.md`](.claude/skills/photo-to-chinese-ink/SKILL.md) 第 4 步 for the exact condition and fallback.
